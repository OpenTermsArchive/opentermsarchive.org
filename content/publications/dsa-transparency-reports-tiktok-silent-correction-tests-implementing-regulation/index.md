...
service: TikTok
terms_types: ["Transparency Report"]
dates: ["2026-08-28"]
...
On 30 August 2026, Open Terms Archive [detected](https://github.com/OpenTermsArchive/dsa-reports-versions/commit/0f2bf601504fe9b4a3e9baa78ba43bc9631e6a49) that TikTok had repointed the download link for its Digital Services Act (DSA) transparency report covering the period July–December 2025. The report had originally been published on 27 February 2026. Without any public announcement, the canonical download URL was changed to point to a new archive labelled “corrected”, while the original archive was left accessible at its previous address.

![Diff showing the detected change](detected_change.png)

The diff shows a single line change: the link for “TikTok DSA Transparency Report (July - December 2025)” was redirected from the original ZIP file to a new path containing `DSA_Corrected_Data_8_28_2026`. No other information on the page was updated to signal that a correction had occurred.

This kind of post-publication edit is exactly the scenario Open Terms Archive’s [DSA systemic risks reports collection](https://opentermsarchive.org/en/dsa-systemic-risks-reports/) was designed to catch. This collection archives every published report independently from the platforms’ own hosting and tracks any modification applied after publication.

Interestingly, in this case, we could catch a _transparency report_ modification even though the collection targets _systemic risks reports_, because both reports are listed on the same page. That meant our analysts could get the information on the change, but could not rely on our archived data and difference detection systems and had to manually investigate.

We thus downloaded both versions of the ZIP archive containing the report and compared them file by file using a GUI diff tool, [Kaleidoscope](https://kaleidoscope.app).
...
> Update on August 28, 2026: The values for specific data points included in tab "4\_notices" of the report published on 27 February 2026 have been updated with the correct values for Category 10d and Category 13b. This update does not affect any other parts of the report.

At Open Terms Archive, our first design principle is “[never trust the platforms](https://docs.opentermsarchive.org/concepts/design-principles/#1-never-trust-the-services)”, and so we proceeded to verify that claim.
...
![Spreadsheet comparison of the original and updated “4\_notices” tab, with row 66 (Category 10d, KEYWORD\_TERRORIST\_CONTENT) highlighted, showing the number of notices received changing from 77729 to 43119](spreadsheet_diff.png)
...
Category 13b (unsafe products) previously contained no data at all and was populated with values in the new report version. As several other lines have no data, one is left to wonder why this line was corrected after the fact and not the others.
...
The XLSX file handling is, on the whole, sensible. The original spreadsheet, `DSA_Transparency_Report_TikTok_July_December_2025.xlsx`, was left in place, and a second file, `DSA_Transparency_Report_TikTok_July_December_2025_updated_28082026.xlsx`, was added alongside it. The original content therefore remains available, which is good practice.

One detail retained our attention: while the original-named XLSX file in the corrected archive is supposed to have stayed the same as the original archive, it is in fact approximately 8% heavier (171 kB versus 159 kB). We compared contents through two different means (visual diffing and statistical comparison) and did not find any change in the data. With the help of two independent LLMs (`GLM-5-2` and `Claude Fable 5.1-medium`), we identified the reason for this file size increase to be a background watermark embedded as a PNG image and displaying the name of the author. That name matches the author recorded in the metadata of the original file. According to public sources, it is TikTok’s EMEA Transparency Program Manager. We assume this watermark to be a traceability mark applied during a re-export of the document for the updated archive, possibly accidentally.

The updated XLSX file also carries a metadata inconsistency. Its first sheet still lists `2026-02-27` as the “Date of the publication of the report”. While technically accurate since the report was indeed first published on that date, this means the only indication of the date of correction is the `_updated_28082026` suffix in the filename, which is not reported within the file itself.
...
Guidelines are clear for transparency reports, the same for systemic risk assessments would be useful!
...
Under DSA §43, the European Commission collects a “supervisory fee” on VLOPSEs. However, the DSA also delegates significant investigative powers to civil society organisations, such as trusted flaggers, academic researchers, consumer protection associations, and open source tools providers. The DSA enforcement team at the European Commission relies on this ecosystem, yet these organisations increasingly struggle with a growing load stemming from an increase in designated platforms, an increase in weaponisation of platforms, a massive usage of LLM-based tools for abusive content creation, and a decrease in funding as historical American philanthropies stop investing due to the Trump administration’s pressure. The DSA was built for a world where platforms would cooperate, civil society organisations were eager, and American capital funded defending fundamental rights online and protecting against hate speech. That world is long gone, yet the duties have remained. European funding needs to flow where European regulation is actually enforced, not only where it is designed.
