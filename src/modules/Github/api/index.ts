...
export const VERSIONS_CONTRIBUTOR_COMMITS_ACTIVITY = 'https://api.github.com/repos/ambanum/OpenTermsArchive-versions/stats/contributors';
export const LAST_VERSIONS_COMMITS = 'https://api.github.com/repos/ambanum/OpenTermsArchive-versions/commits';
...
export const getLastVersionsCommits = async () => {
  try {
    const per_page = 10;
    const  { data } : { data: Commits } = await octokit.request(`GET ${LAST_VERSIONS_COMMITS}`, { per_page });
    return data;
  } catch (e) {
    console.error(e);
    return {};
